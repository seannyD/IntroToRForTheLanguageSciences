...
**NOTE**: The "../" part at the beginning of the file location means "look in the folder above this one". The current working directory is inside the analysis folder, and the data we want is in the **data** folder, which is one step above this. So we're saying "go into the parent folder, find the folder called "data", and inside that find the folder called "WALS_WordOrder.csv". This might seem complicated, but now this line of code will work on anyone's computer (if they have the data). We've just made our code more *reproducible*!
